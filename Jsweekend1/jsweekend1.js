...
var pricenum = items.map(function(total){
  return total.price;});

total = pricenum.reduce(function(a , b) {
  return a + b; }) / pricenum.length;
...
var costRange = items.filter(function(between) {
  return between.price > 14 && between.price < 18;});
  costRange.map(function(between2) {
   return between2.title;});
 console.log("Items that cost betwen $14.00 and $18.00");
costRange.forEach(function(costRange2) {console.log("title:" + costRange2.title);});

/* Question 3 */

var gbp = items.filter(function(gbp2){
  return gbp2.currency_code == "GBP";});

gbp.map(function(answer){
  return answer.title + " costs " + "£" + answer.price;})[0];

/* Question 4*/

items.filter(function(woody) {
 if (woody.materials.indexOf("wood") >= 0) {
 console.log(woody.title + " is made of wood");}});

/* Question 5 */

var answer5 = items.filter(function(eight) {
 if (eight.materials.length >= 8) {
console.log(eight.title + " has " + eight.materials.length + eight.length + " materials.");
eight.materials.forEach(function(eightmaterials) {
...
var madeSeller = items.filter(function(made) {
  return made.who_made == "i_did";});
 console.log(madeSeller.length + " were made by their sellers");
